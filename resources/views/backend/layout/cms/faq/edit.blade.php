...
<form action="{{ route('faq.update', $faq->slug) }}" method="POST">
  ...
  {{--   <div class="mb-3">
        ...
        <input type="text" class="form-control" id="question" name="question" placeholder="Enter question" value="{{ old('question', $faq->question) }}" required>
    </div> --}}

    <div class="mb-3">
        <label for="answer" class="form-label">Answer</label>
        <textarea class="form-control" id="answer" name="answer" rows="6" placeholder="Enter answer">{{ old('answer', $faq->answer) }}</textarea>
    </div>
